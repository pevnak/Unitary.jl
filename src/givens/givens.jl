...
struct TransposedUnitaryGivens{B<:UnitaryGivens} 
	parent::B
end

Flux.@functor UnitaryGivens
Flux.@functor TransposedUnitaryGivens

function Base.Matrix(a::UnitaryGivens{T}) where {T}
	a * T.(Matrix(I(a.n))) 
end

Base.Matrix(a::TransposedUnitaryGivens) = transpose(Matrix(a.parent))


Base.size(a::UnitaryGivens) = (a.n,a.n)
Base.size(a::UnitaryGivens, i::Int) = a.n
Base.size(a::TransposedUnitaryGivens,i...) = size(a.parent)

Base.eltype(a::UnitaryGivens{T}) where {T} = T
Base.eltype(a::TransposedUnitaryGivens) = eltype(a.parent)
LinearAlgebra.transpose(a::UnitaryGivens) = TransposedUnitaryGivens(a)
LinearAlgebra.transpose(a::TransposedUnitaryGivens) = a.parent
Base.inv(a::UnitaryGivens) = transpose(a)
Base.inv(a::TransposedUnitaryGivens) = transpose(a)
Base.show(io::IO, a::UnitaryGivens) = print(io, "UnitaryGivens ",a.θs)
Base.show(io::IO, a::TransposedUnitaryGivens) = print(io, "UnitaryGivensᵀ ",a.parent.θ)
Base.zero(a::UnitaryGivens) = UnitaryGivens(zero(a.θs), a.i, a.j, a.n)
Base.zero(a::TransposedUnitaryGivens) = TransposedUnitaryGivens(zero(a.parent))


Zygote.@adjoint function LinearAlgebra.transpose(x::TransposedUnitaryGivens)
  return(transpose(x), Δ -> (TransposedUnitaryGivens(Δ.θ),))
end

Zygote.@adjoint function LinearAlgebra.transpose(x::UnitaryGivens)
  return transpose(x), Δ -> (UnitaryGivens(Δ.parent.θs, x.idxs, x.n),)
end



*(a::UnitaryGivens, x::TransposedMatVec) = (@assert a.n == size(x,1); _mulax(a.θs, a.idxs, x, 1))
*(x::TransposedMatVec, a::UnitaryGivens) = (@assert a.n == size(x,2); _mulxa(x, a.θs, a.idxs, 1))
*(a::TransposedUnitaryGivens, x::TransposedMatVec) = (@assert a.parent.n == size(x,1); _mulax(a.parent.θs, a.parent.idxs, x, -1))
*(x::TransposedMatVec, a::TransposedUnitaryGivens) = (@assert a.parent.n == size(x,2); _mulxa(x, a.parent.θs, a.parent.idxs, -1))
